...
def extract_new_xml_data(root):
    ...
    translation = [float(x)
                   for x in root.find('Translation/data').text.split()]
...
def rgbd2pcd(color, depth, intr, extr, dt):
    source_color = o3d.io.read_image(color)
    source_depth = o3d.io.read_image(depth)
    pinhole_cam_intr = intr
    rgbd_image = o3d.geometry.RGBDImage.create_from_color_and_depth(
        source_color, source_depth, depth_trunc=dt, convert_rgb_to_intensity=False)  # depth_trunc=10
    pcd = o3d.geometry.PointCloud.create_from_rgbd_image(
        rgbd_image, pinhole_cam_intr, extr)
    return pcd
...
def split_data(data_folder):
    ...
    files_dat_color = sorted(
        [item for item in files if 'color' in item], key=sort_func)
    files_dat_depth = [item.replace("color", "depth").replace(
        "bmp", "png") for item in files_dat_color]
...
def img2pcd(img, depth, intr, extr, output_file, dt=10.):
    ...
    pcd = rgbd2pcd(img, depth, intrinsic, inv_extr, dt=dt)
